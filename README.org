...
being used to store the return value, to *lock* the two together: =int
*x = malloc(length * sizeof *x);= This also moves the =lengh= to the
front for increased visibility, and drops the redundant parentheses
with =sizeof()=; they are only needed when the argument is a type
name. Many people seem to not know or ignore this, which makes their
code move verbose. Remember: =sizeof= is not a function!
...
should be better to write the expression as: =int *x = malloc(sizeof
*x * length);= Compare with =malloc(sizeof *x * length * width)= vs.
