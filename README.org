...
The scope of a [declaration](#declaration-and-definition) is the part
of the code where the declaration is seen and can be used. Note that
this says nothing about whether the object associated to the
declaration can be accessed from some other part of the code via
another declaration. We uniquely identify an object by its memory: the
storage for a variable or the function code.

Finally, note that a [declaration](#declaration-and-definition) in a
nested scope can hide a declaration in an outer scope; but only if one
of two has no [linkage](#linkage).


*** Duration
		:PROPERTIES:
		:CUSTOM_ID: language_scope_duration
		:END:

Indicates whether the object associated to the
[declaration](#declaration-and-definition) persists throughout the
program's execution or whether it is allocated dynamically when the
declaration's scope is entered.

*** Linkage
		:PROPERTIES:
		:CUSTOM_ID: language_scope_linkage
		:END:
...
- No Linkage: A declaration with no linkage is associated to an object
  that is not shared with any other declaration. All declarations with
  no linkage happen at block scope: all block scope declarations
  without the extern storage class specifier have no linkage.
...
- External Linkage-: within the whole program, all declarations with
  external linkage for the same identifier refer to the same object.
...
scope](#file-scope).  They are visible from their
[declaration](#declaration-and-definition) until the end of the
