...
static void test_lookahead (char *);
static void test_pushback (char *);
static void test_lexan (char *);
      ...
      else if (ispunct (c))
        {
          switch (c)
            ...
            case '/':
              ...
              return DIV;
            default:
              error ("!panic, invalid character: '%s'\n", c);
            };
        }
      else if (EOF == c)
        {
...
int
insert (char *s, enum token_e tok)
{
